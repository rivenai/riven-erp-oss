...
class ThreadedWSGIServerReloadable(LoggingBaseWSGIServerMixIn, werkzeug.serving.ThreadedWSGIServer):
    ...
    def server_bind(self):
        ...
        if os.environ.get('LISTEN_FDS') == '1' and os.environ.get('LISTEN_PID') == str(os.getpid()):
            ...
            _logger.info('Riven ERP HTTP service (werkzeug) running through socket activation')
        else:
            self.reload_socket = False
            super(ThreadedWSGIServerReloadable, self).server_bind()
            _logger.info('Riven ERP HTTP service (werkzeug) running on %s:%s', self.server_name, self.server_port)
    ...
    def start(self):
        ...
        if config['http_enable']:
            if config.http_socket_activation:
                ...
                _logger.info('Riven ERP HTTP service (werkzeug) running on %s:%s', self.interface, self.port)
...
class Worker(object):
    ...
    def setproctitle(self, title=""):
        setproctitle('riven-erp: %s %s %s' % (self.__class__.__name__, self.pid, title))
...
def start(preload=None, stop=False):
    """ Start the Riven ERP http server and cron processor.
    """
    global server
